...
for C = 1 : numberOfCannabisImagesToProcess
	...
	fullImageFileName = fullfile(CannabisImagesFolder, baseFileName);
        ...
        filterT = sortedT.Area > 10000 & sortedT.Area < 85000;
        
        sortedT= sortedT(filterT,:);
        filterT =  sortedT.BoundingBox(:,1) > 0.6 &  sortedT.BoundingBox(:,1) > 0.6 & sortedT.BoundingBox(:,3) < max(T.BoundingBox(:,3)-300);
        sortedT= sortedT(filterT,:);
...
saveas(gcf,'7.1_leaves_segmentation.jpg','jpg');
leaves_segmentation_img=imread('7.1_leaves_segmentation.jpg');
close();
        fprintf('STEP7: leaves segmentation done...\n')
        %%
        ...
        %Break for correctly analyze image
        close all;
        imtool close all;
        n = 3;
        fprintf('Cleaning space before image splitting, a 3 seconds pause will be introduced... \n');
        pause(n);
...
done_folder='done';
movefile(fullImageFileName,done_folder);
movefile(image_name,done_folder);
fprintf('COMPLETED FILE MOVED TO DONE FOLDER...\n')
